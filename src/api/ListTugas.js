...
const ListTugas = async (token, refreshToken,id_kelas) => {
  try {
    // Request pertama dengan token lama
    const response = await axios.get(`${url}tugas/${id_kelas}`, {
      headers: {
    ...
    if (error.response?.status === 401) {
      try {
        ...
        const retryResponse = await axios.get(`${url}tugas/${id_kelas}`, {
          headers: {
