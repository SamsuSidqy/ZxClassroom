...
const MyAsigsments = async (token, refreshToken,id_kelas) => {
  try {
    // Request pertama dengan token lama
    const response = await axios.get(`${url}asigsment/${id_kelas}`, {
      headers: {
    ...
    if (error.response?.status === 401) {
      try {
        ...
        const retryResponse = await axios.get(`${url}asigsment/${id_kelas}`, {
          headers: {
