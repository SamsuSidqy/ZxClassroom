...
const DetailTugas = async (token, refreshToken,id_tugas) => {
  try {
    // Request pertama dengan token lama
    const response = await axios.get(`${url}tugas/detail/${id_tugas}`, {
      headers: {
    ...
    if (error.response?.status === 401) {
      try {
        ...
        const retryResponse = await axios.get(`${url}tugas/detail/${id_tugas}`, {
          headers: {
