...
const UpdateKelas = async (token, refreshToken,body,kode) => {
  try {
    // Request pertama dengan token lama
    const response = await axios.put(`${url}kelas/${kode}`, body,{
      headers: {
    ...
    if (error.response?.status === 401) {
      try {
        ...
        const retryResponse = await axios.put(`${url}kelas/${kode}`, body,{
          headers: {
