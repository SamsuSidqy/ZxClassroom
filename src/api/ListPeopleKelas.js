...
const ListPeopleKelas = async (token, refreshToken,kodeKelas) => {
  try {
    // Request pertama dengan token lama
    const response = await axios.get(`${url}people/${kodeKelas}`, {
      headers: {
    ...
    if (error.response?.status === 401) {
      try {
        ...
        const retryResponse = await axios.get(`${url}people/${kodeKelas}`, {
          headers: {
...
export default ListPeopleKelas